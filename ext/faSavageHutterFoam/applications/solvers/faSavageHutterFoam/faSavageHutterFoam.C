...
#include "readSolutionControls.H"
  ...
  Info << nl << "Numerical settings" << nl << "    max number of iterations "
       << nCorr << nl << "    min number of iterations " << minCorr << nl
       << "    TOL h " << hResidualMax << nl << "    TOL Us " << UsResidualMax
       << nl << endl;
...
#include "calcBasalstress.H"
      ...
      faVectorMatrix UsEqn(fam::ddt(h, Us) + xi * fam::div(phi2s, Us) + tauSc +
                               fam::Sp(tauSp, Us) ==
                           gs * h - fac::grad(pb * h / (2. * friction->rho())));
          ...
          if (diag[i] < 1e-8) {
            ...
            while (faceIndex < endFaceNbr) {
              source[owner[losort[faceIndex]]] -=
                  upper[losort[faceIndex]] * forcedValue;
      ...
      faScalarMatrix hEqn(
          fam::ddt(h) + fam::div(phis, h) ==
          Sm - fam::Sp(Sd / (h + dimensionedScalar("small", dimLength, SMALL)),
                       h));
      ...
      faScalarMatrix hentrainEqn(
          fam::ddt(hentrain) ==
          Sd - fam::Sp(Sm / (hentrain +
                             dimensionedScalar("small", dimLength, SMALL)),
                       hentrain));

      hentrainEqn.solve();

      // output entrainment component
      W = Sm;
      ...
      if (final) {
        if (hResidual.initialResidual() < hResidualMax &&
            mag(UsResidual.initialResidual()) < UsResidualMax) {
          Info << "reached residual in h = " << hResidual.initialResidual()
               << " < " << hResidualMax
               << " and in Us = " << UsResidual.initialResidual() << " < "
               << UsResidualMax << ", stopping loop!" << endl;
        } else {
