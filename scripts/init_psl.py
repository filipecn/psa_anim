...
foam_file_dict_template = "FoamFile\n{\n" \
                          "\tversion     2.0;\n" \
                          "\tformat      ascii;\n" \
                          "\tclass       dictionary;\n" \
                          "%s\n}\n" \
...
def write_control_dict_file(path, delta_t, write_time, end_time, write_format, template_file):
    with open(template_file, "r") as f:
        file_content = f.read()
    file_content = file_content.replace("<<<APPLICATION>>>", "pslFoam") \
...
def write_transport_properties(path, snow_density, air_density, template_file, dab, snow_viscosity):
    with open(template_file, "r") as f:
        file_content = f.read()
    file_content = file_content.replace("<<<SNOW_DENSITY>>>", str(snow_density)) \
...
def write_turbulence_properties(path, use_turbulence, template_file):
    ...
    if use_turbulence:
        file_content = file_content.replace("<<<SIMULATION_TYPE>>>", "RAS") \
            .replace("<<<CONFIG>>>",
                     "RAS\n"
                     "{\n"
                     "  // Mandatory entries\n"
        ...
        file_content = file_content.replace("<<<SIMULATION_TYPE>>>", "laminar") \
            .replace("<<<CONFIG>>>", "")
...
def write_fv_schemes(path, use_turbulence, template_file):
    div_schemes = { "div(rhoPhi,U)": "Gauss vanLeer;", # "Gauss linear;",
...
def write_decompose_par_dict(path, x, y, z, template_file):
    with open(template_file, "r") as f:
        file_content = f.read()
    file_content = file_content.replace("<<<NUMBER_OF_SUBDOMAINS>>>", str(x * y * z)) \
...
def write_psl_control(path, args):
    ...
    file_content += "pslEntrainmentMethod " + str(args.entrainment_method) + ";            // entrainment method [0 2]\n"
    file_content += "pslSnowCoverDensity 25;            // snow cover density [kg / m3]\n"
...
def write_boundary_conditions(domain_type, use_turbulence, template_path, path, run_2d):
    ...
    input_fields = ["alpha_inj", "alpha_inj_U", "dsl_U", "dsl_front_sdf", "dsl_h", "dsl_slope", "dsl_W", "dsl_divU", "Uinj", "gradAlpha1"]
    ...
    for variable in conditions:
        ...
        for patch in conditions[variable]:
            ...
            for entry in conditions[variable][patch]:
                condition += "\t\t\t" + entry + "\t\t\t" + conditions[variable][patch][entry] + ";\n"
            file_content = file_content.replace("<<<" + patch.upper() + "_CONDITIONS>>>", condition)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--write-format", type=str, help="output file type (ascii|binary)", default="ascii")
    ...
    parser.add_argument("--snow-density", type=float, help="snow cloud density kg/m3", default=7)
    parser.add_argument("--air-density", type=float, help="air density kg/m3", default=1.225)
    ...
    parser.add_argument("--domain-type", type=str, help="openBox | tunnel", default="openBox")
    ...
    parser.add_argument("--u-factor", type=float, help="velocity entrainment factor", default=1)
    parser.add_argument("--a-factor", type=float, help="mass entrainment factor", default=1)
    parser.add_argument("--f-factor", type=float, help="front length factor", default=10)
    ...
    write_control_dict_file(system_dir, args.dt, args.wt, args.et, args.write_format,
                            args.template_dir / "system/controlDict")
    write_transport_properties(constant_dir, args.snow_density, args.air_density,
                               args.template_dir / "constant/transportProperties", args.dab, args.snow_viscosity)
    write_turbulence_properties(constant_dir, args.use_turbulence,
                                args.template_dir / "constant/turbulenceProperties")
    write_fv_schemes(system_dir, args.use_turbulence, args.template_dir / "system/fvSchemes")
    write_fv_solution(system_dir, args.use_turbulence, args.template_dir / "system/fvSolution")
    write_decompose_par_dict(system_dir, args.px, args.py, args.pz, args.template_dir / "system/decomposeParDict")
    write_all_run(args.o, args.set_fields, args.mesh_generator, args.template_dir / "Allrun")
    write_all_run(args.o, args.set_fields, args.mesh_generator, args.template_dir / "Allrun-parallel")
    write_boundary_conditions(args.domain_type, args.use_turbulence, args.template_dir / "0.orig", variables_dir, args.run_2d)
