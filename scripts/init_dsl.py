...
def write_transport_properties(path, args, template_file):
    ...
    LOG("[init_dsl] writing ...", path / "transportProperties")
...
def write_control_dict_file(path, delta_t, write_time, end_time):
    ...
    with open(control_file_path, "r") as f:
        file_content = f.read()
    file_content = (
        file_content.replace("<<<END_TIME>>>", "%.8f" % end_time)
        .replace("<<<DELTA_T>>>", "%.8f" % delta_t)
        .replace("<<<WRITE_INTERVAL>>>", "%.8f" % write_time)
    )

    LOG("[init_dsl] writing ...", control_file_path)
...
def write_all_run(path, mesh_generator, template_file, dem):
    ...
    LOG("[init_dsl] writing ...", path / os.path.basename(template_file))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--snow-density", type=float, help="snow cloud density kg/m3", default=20
    )
    parser.add_argument(
        "--mesh-generator",
        type=str,
    ...
    if args.o.exists() and os.listdir(args.o):
        ERR("[init_dsl] Output folder not empty")
        exit(1)
    ...
    write_transport_properties(
        constant_dir, args, args.template_dir / "constant/transportProperties"
    )
    ...
    write_all_run(
        args.o, args.mesh_generator, args.template_dir / "Allrun-parallel", args.dem
    )
